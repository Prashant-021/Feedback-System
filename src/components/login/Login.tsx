...
const Login: React.FC = () => {
  ...
  const { values, touched, errors, handleBlur, handleChange, handleSubmit, setFieldError } = useFormik({
    ...
    onSubmit: (values: currentUser) => {
      ...
      } else { setFieldError('Email', 'User not found') }
    }
  })
  return (
            ...
            <div className='w-screen md:w-1/2 bg-white p-5 flex flex-col justify-center items-center'>
                ...
                <form action="" onSubmit={handleSubmit}>
                    <div className="inputGroup py-2 mb-2 ">
                        <Input label='Email' name='Email' type="text" value={values.Email} onChange={handleChange} onBlur={handleBlur}/>
                        {Boolean(errors.Email) && Boolean(touched.Email) && (<Typography variant="small" color="red" className="absolute">{errors.Email}</Typography>)}
                    </div>
                    <div className="inputGroup pt-2 pb-3">
                        <Input label='Password' name='password' type={showPassword ? 'text' : 'password'} value={values.password} onChange={handleChange} onBlur={handleBlur} />
                        {/*  error={!!(errors.password && touched.password)} */}
                        {Boolean(errors.password) && Boolean(touched.password) && (<Typography variant="small" color="red" className="absolute">{errors.password}</Typography>)}
                    </div>
                    <div className="mt-5">
