...
const CategoriesList: React.FC = () => {
  const TABLE_ROWS = useSelector((state: RootState) => state.category.category)
              ...
              <tr>
                {TABLE_HEAD.map((head) => (
                  <th key={head} className="border-b  border-blue-100  bg-blue-500 p-4">
                    <Typography color="white" className="font-normal leading-none">
                      {head}
                    </Typography>
                  </th>
                ))}
              ...
              {TABLE_ROWS.map((category) => (
                <tr key={category.title} className="even:bg-blue-gray-50/50">
                  ...
                  <td className="p-4 flex justify-evenly">
                    ...
                    <Button variant='text' color="red" className="font-medium" onClick={() => dispatch(deleteCategory(category.title))}>
                      Delete
                    </Button>
                  </td>
                </tr>
